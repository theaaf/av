...
public:
    ...
    virtual std::shared_ptr<EncodedAVReceiver> authenticate(const std::string& connectionId) override {
        ...
        return std::make_shared<Archiver>(_logger.with("connection_id", connectionId), _archiveBucket, fmt::format("{}/{}/{}/{}", gmTime.tm_year + 1900, gmTime.tm_mon + 1, gmTime.tm_mday, connectionId)+"/{}");
    }
