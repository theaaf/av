...
#define STR2AVAL(av, str) av.av_val = const_cast<char*>(str); av.av_len = strlen(av.av_val) // NOLINT(cppcoreguidelines-pro-type-const-cast,misc-macro-parentheses)
#define SAVCX(name, x) static const AVal name = {const_cast<char*>(x), sizeof(x)-1} // NOLINT(cppcoreguidelines-pro-type-const-cast,misc-macro-parentheses)
...
} _rtmpLogger; // NOLINT(cert-err58-cpp)
...
bool RTMPConnection::_serveInvoke(RTMP* rtmp, RTMPPacket* packet, const char* body, size_t len) {
    ...
    if (AVMATCH(&method, &av_createStream)) {
        return _sendResultNumber(rtmp, txn, _nextStreamId++);
    }
    if (AVMATCH(&method, &av_releaseStream)) {
        return true;
    }
    if (AVMATCH(&method, &av_FCPublish)) {
        AVal name{};
